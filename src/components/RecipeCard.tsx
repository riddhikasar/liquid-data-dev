...
export function RecipeCard({ recipe, index }: { recipe: Recipe; index?: number }) {
  ...
  return (
    ...
    >
      ...
      {/* Oversized drink illustration — overflows the card edges, anchored far right */}
      <img
        src={drink}
        alt=""
        loading="lazy"
      ...
      <div className="absolute top-5 right-5 text-slate-900/70 text-xs font-semibold tracking-wider z-10">
        {typeof index === "number" ? `#${String(index + 1).padStart(2, "0")}` : ""}
      </div>
      <div className="relative flex flex-col h-full justify-between text-slate-900 p-6 z-10">
        <div>
          <h3 className="font-display text-2xl font-bold max-w-[58%]">{recipe.name}</h3>
          <p className="text-sm text-slate-900/75 mt-1 italic max-w-[55%]">{recipe.tagline}</p>
        </div>
        <p className="text-sm text-slate-900/90 max-w-[55%] leading-snug">
